class MetricsController < ApplicationController
  ...
  def index
    ...
    # Aperturas diarias por locker
    today = Time.current.beginning_of_day..Time.current.end_of_day
    @daily_openings_per_locker = @lockers.each_with_object({}) do |locker, hash|
      hash[locker.id] = locker.openings.where(opened_at: today).count
    end

    # Total de aperturas diarias
    @daily_openings_total = @daily_openings_per_locker.values.sum

    # Aperturas diarias para los últimos 7 días
